...
const Intro = (): React.JSX.Element => {
    ...
    const images = [
        { image: dach, link: de.anchors.dach },
        { image: haustechnik, link: de.anchors.haustechnik },
        { image: heizung, link: de.anchors.heizung },
        { image: bad, link: de.anchors.bad }
    ]

    return (
        <div className="sm:w-full h-screen bg-white flex justify-center items-center flex-col px-5">
            <div className="pb-2 sm:mt-0 mt-24 w-auto sm:text-left text-center text-4xl"> {de.site.tagline}</div>
            <div className="sm:ml-0 ml-4 grid grid-cols-2 gap-1 w-auto h-auto">
                {images.map((item, index) => (
                    <a key={index} href={`#${item.link}`}>
                        <Image alt={de.media.imageAlt} className="w-full duration-300 ease-in-out hover:scale-110" src={item.image} />
                    </a>
                ))}
            </div>
        </div>)
